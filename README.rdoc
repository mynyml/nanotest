<em>"Frustra fit per plura quod potest fieri per pauciora."</em> --William of Ockham
...
above, or use its method directly: <tt>Nanotest.assert { true }</tt>. Its
block is expected to return a boolean. If it's false it fails, otherwise it
passes. Simple as that.
